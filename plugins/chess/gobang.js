...
function start(data) {
  ...
  if (gobang_battle.has(group_id)) return reply('当前群聊对局尚未结束，请勿重复开启');
  ...
  gobang_battle.set(group_id, battle);
...
function move(data) {
  ...
  if (!gobang_battle.has(group_id)) return reply('当前群聊未开启对局');

  const battle = gobang_battle.get(group_id);
...
function surrender(data, msg = `${data.sender.card ? data.sender.card : data.sender.nickname} 认输`) {
  ...
  if (!gobang_battle.has(group_id)) return;
  const { timeout } = gobang_battle.get(group_id);
