...
function checkAircon(bot, group_id) {
    return new Promise((resolve, reject) => {
        bot.getGroupInfo(group_id)
            ...
            // 空调是否安装
            !all_aircon.has(group_id) && all_aircon.set(group_id, new class_1.Aircon(max_member_count, member_count));
...
function aircon(bot, data) {
    ...
    class_1.checkCommand(raw_message, aircon.open) && open(bot, data);
    class_1.checkCommand(raw_message, aircon.close) && close(bot, data);
    class_1.checkCommand(raw_message, aircon.adjust) && adjust(bot, data);
    class_1.checkCommand(raw_message, aircon.show) && show(bot, data);
}
