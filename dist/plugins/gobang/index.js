...
function start(data) {
    ...
    if (all_battle.has(group_id))
        return reply('当前群聊对局尚未结束，请勿重复开启');
    // 获取棋盘大小，默认 10 * 10
    const board = [];
    ...
    const battle = new class_1.Battle(user_id, board, timeout);
...
function move(data) {
    ...
    if (msg)
        return reply(msg);
    // 落子坐标
    const x = raw_message.match(reg.x)?.join().toUpperCase().charCodeAt(0) - 64;
    const y = Number(raw_message.match(reg.y)?.join(''));
    // 已有棋子
    ...
    // 坐标越界
    if (!board[x][y])
        return reply(`[CQ:at,qq=${user_id}] 越界了，亲 (╯▔皿▔)╯`);
    board[x][y] = offensive ? '●' : '○';
    battle.board = board;
    // 记录落子
...
function surrender(data, msg = `${data.sender.card ? data.sender.card : data.sender.nickname} 认输`) {
    const { group_id, reply } = data;
    if (!all_battle.has(group_id))
        return;
    const { timeout } = all_battle.get(group_id);
...
function gobang(bot, data) {
    ...
    class_1.checkCommand(raw_message, gobang.start) && start(data);
    class_1.checkCommand(raw_message, gobang.move) && move(data);
    class_1.checkCommand(raw_message, gobang.rollback) && rollback(data);
    class_1.checkCommand(raw_message, gobang.surrender) && surrender(data);
    class_1.checkCommand(raw_message, gobang.over) && over(data);
}
