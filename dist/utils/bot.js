...
exports.getLevel = exports.checkGroup = exports.checkCommand = exports.Bot = void 0;
...
async function checkGroup(bot, all_plugin) {
    ...
    const exists = util_1.checkFileSync(`${path.groups}/${uin}.yml`);
    const params = await util_1.getProfile('params');
    const groups = exists ? await util_1.getProfile(uin.toString(), path.groups) : {};
    !exists && fs_1.writeFile(`${path.groups}/${uin}.yml`, '', err => err && logger.error(err));
    // 获取群信息
    bot.groups = groups;
    bot.gl.forEach((val) => {
