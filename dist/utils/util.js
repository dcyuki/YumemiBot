 ...
 * @returns Promise 对象
 */
function setProfile(file_name, data, file_folder = `${__yumeminame}/config`) {
 ...
 */
function getProfile(file_name, file_folder = `${__yumeminame}/config`) {
    return new Promise((resolve, reject) => {
        const file_path = `${file_folder}/${file_name}.yml`;
 ...
 * @returns 返回 JSON 对象
 */
function getProfileSync(file_name, file_folder = `${__yumeminame}/config`) {
    const file_path = `${file_folder}/${file_name}.yml`;
    try {
